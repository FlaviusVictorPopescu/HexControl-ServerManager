...
export function DomainTable({
  ...
  domains: Domain[];
  onEdit: (domain: Domain, patch: Partial<Domain>) => void;
        ...
        <TableBody>
          {items.map((d) => (
            <TableRow key={d.id}>
              ...
              <TableCell>
                <Badge variant="secondary">{d.nodeVersion}</Badge>
              </TableCell>
              <TableCell>
                {d.sslEnabled ? (
                  <Badge
              ...
              <TableCell>
                {d.dockerContainer ? (
                  <Badge variant="outline">{d.dockerContainer}</Badge>
                ) : (
              ...
              <TableCell>
                {d.nginxProxy ? (
                  <span className="text-muted-foreground">{d.nginxProxy}</span>
                ) : (
                  <span className="text-muted-foreground">—</span>
                )}
              </TableCell>
              <TableCell className="text-right space-x-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    onEdit(d, { autoSslEnabled: !(d as any).autoSslEnabled })
                  }
                >
...
function Expiry({ domain }: { domain: string }) {
  ...
  React.useEffect(() => {
    let active = true;
    Api.certExpiry(domain)
      .then((r) => active && setExp(r.expiresAt))
      .catch(() => active && setExp(null));
    return () => {
      active = false;
    };
  }, [domain]);
  ...
  return (
    <span
      className={
        days < 15 ? "text-red-600 text-xs" : "text-xs text-muted-foreground"
      }
    >
