...
export function FileManager({ domain }: { domain: Domain }) {
  ...
  const { data: files = [], isLoading } = useQuery({
    queryKey: ["files", domain.id],
    queryFn: () => Api.listFiles(domain.id),
  });
  ...
  const upload = useMutation({
    mutationFn: () =>
      Api.uploadFile(domain.id, {
        name,
        size: Math.floor(Math.random() * 5000) + 100,
        path: `/var/www/${domain.name}/${name}`,
      }),
    onSuccess: () => {
      setName("");
      qc.invalidateQueries({ queryKey: ["files", domain.id] });
    },
  });
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex flex-col md:flex-row gap-3 md:items-end">
          ...
          <Button onClick={() => upload.mutate()} disabled={!name}>
            Upload
          </Button>
        </div>
