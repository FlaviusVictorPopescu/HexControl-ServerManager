...
export const Api = {
  ...
  dockerList: () => http<import("@shared/api").DockerContainerInfo[]>("/api/docker/containers"),
  dockerInstall: () => fetch("/api/docker/install", { method: "POST" }).then(() => {}),
  dockerCreate: (name: string, image: string, portHost?: number, portContainer?: number) => http<{ status: string }>("/api/docker/create", { method: "POST", body: JSON.stringify({ name, image, portHost, portContainer }) }),
  dockerDelete: (id: string) => http<{ status: string }>("/api/docker/delete", { method: "POST", body: JSON.stringify({ id }) }),
  dockerAssign: (domain: string, port: number) => http<{ status: string }>("/api/docker/assign", { method: "POST", body: JSON.stringify({ domain, port }) }),
