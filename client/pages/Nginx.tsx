...
export default function NginxPage() {
  ...
  const sites = useQuery({ queryKey: ["nginx","sites"], queryFn: Api.listNginxSites });
  const config = useQuery({ queryKey: ["nginx","config"], queryFn: Api.getNginxConfig });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="space-x-2">
          <Button variant="outline" onClick={() => restart.mutate()}>Restart</Button>
          <ProxyDialog onSaved={() => qc.invalidateQueries({ queryKey: ["nginx"] })} />
        </div>
      </div>

      <Card>
        <CardHeader><CardTitle>Enabled Sites</CardTitle></CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader><CardTitle>nginx.conf</CardTitle></CardHeader>
        <CardContent>
          <pre className="max-h-96 overflow-auto rounded-md bg-muted p-4 text-xs whitespace-pre-wrap">{config.data}</pre>
        </CardContent>
      </Card>
    </div>
  );
...
function ProxyDialog({ onSaved }: { onSaved: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader><DialogTitle>Configure Proxy</DialogTitle></DialogHeader>
        <div className="grid gap-3">
          <div className="grid gap-2">
            <Label htmlFor="d">Domain</Label>
            <Input id="d" placeholder="example.com" value={domain} onChange={(e) => setDomain(e.target.value)} />
          </div>
          <div className="grid gap-2">
