...
export async function issueSslForDomain(
  name: string,
  email = process.env.CERTBOT_EMAIL || "admin@hexbit.ro",
) {
  ...
  try {
    ...
    pushActivity({
      kind: "ssl.issued",
      message: `Certbot attempted for ${safe}`,
      meta: { stdout, stderr },
    });
  } catch (e: any) {
    pushActivity({
      kind: "ssl.failed",
      message: `Certbot failed for ${safe}`,
